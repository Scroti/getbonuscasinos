...
function buildWelcomeEmail(domain: string, siteName: string): string {
          ...
          <tr>
            <td style="padding:36px 32px;">
              ...
              <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#a1a1aa;">
                Thanks for subscribing. You'll be the first to hear about the hottest casino bonuses, exclusive no-deposit offers, and free spins — curated and updated daily.
              </p>
          ...
          <tr>
            <td style="padding:20px 32px 28px;border-top:1px solid rgba(255,255,255,0.06);text-align:center;">
              ...
              <p style="margin:0;font-size:11px;color:#52525b;">
                You're receiving this because you subscribed on ${siteName}. &nbsp;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Save to Firestore
    const result = await subscribeToNewsletter(email.toLowerCase().trim(), profileData);
    ...
    if (process.env.RESEND_API_KEY) {
      await resend.emails.send({
        ...
        html: buildWelcomeEmail(domain, siteName),
      });
    }
